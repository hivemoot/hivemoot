...
def test_matches_docs_dir_rst():
    # Any file under docs/ matches docs/** (not just .md)
    assert matches("docs/reference.rst", ALLOWED) is True
...
def test_denied_denylist_bypass_attempt_adjacent():
    # Path adjacent to .github but not inside it — not on denylist, not on allowlist
