...
POST /api/rooms/:id/confirm-merge
   ...
   → server compares current_head_sha against the reviewed_head_sha
     ...
     transitions to closed; audit decision_outcome=merge_completed)
...
local_queen:
  ...
  # New for local mode:
  - rooms.synthesize          # NEW — additive per D14, gates GET /api/rooms/synthesis-ready, GET /api/rooms/decided-pending-ready, POST claim-synthesis, POST resolve-action, POST seal-decision, POST confirm-merge
...
Guard pass-7: the tick-N+1 head-SHA re-comparison + merge confirmation is a distinct security gate from `seal-decision`'s comment-URL verification, with different idempotency semantics (G28 below) and different inputs (`current_head_sha` from a fresh GitHub read, not `comment_url`). PR 3 introduces `POST /api/rooms/:id/confirm-merge` gated by `rooms.synthesize`; body carries `{ merge_attempt_id, current_head_sha }`. Server compares `current_head_sha` against the `reviewed_head_sha` stored against the `decided_pending_action` claim and either approves (room transitions to `closed` immediately, audit `decision_outcome: merge_completed` meaning "merge decision approved" — the local queen then runs `gh pr merge --squash` and a separate audit field `github_merge_status: pending|succeeded|failed` is written after the actual merge attempt) or downgrades (room transitions to `closed`, audit `decision_outcome: merge_downgraded` + `reason: head_sha_drift_at_tick_n1` or other re-validation failure). **Pre-merge close model** (drone pass-21): the room transitions before `gh pr merge --squash` runs, simplifying the contract to one server roundtrip per tick. A failed `gh pr merge --squash` after approval is detectable via the `github_merge_status` audit field + a reconciliation signal in PR 5's dashboard (room shows `merge_completed` but PR is still open on GitHub). Keeps the security gate uniform and removes the need for the local queen to read its own audit rows.
