...
| 4 | Hive queen feature block under `plugins.hivemoot.queen`. Two trigger loops: (a) **synthesis loop** polls `synthesis-ready`, applies D5's race mitigations, captures `reviewed_head_sha` via fresh `gh pr view --json headRefOid` after `claim-synthesis` (builder pass-9 §3 + G21), runs two local `generateObject` calls (D2/D3) on Codex flat cost, submits to `resolve-action`, posts comment, calls `seal-decision`; downgrade-comment retries with backoff (G34) emit `queen.downgrade_comment_failed` audit on persistent failure; (b) **confirm-merge loop** polls `decided-pending-ready` (G27, separate query), captures fresh `current_head_sha`, generates `merge_attempt_id` (G28, separate from `audit_id`), calls `confirm-merge` (G25), runs `gh pr merge --squash`, calls `report-merge-result` (G33). Restart-resilient — no local state persistence required (G26). Loads structured override config (D12). Operator setup guide pinned to require GitHub branch protection per G31. | Depends on PR 3 |
...
Guard pass-9: G30 alone is **detection** — it tells an operator at 3am that a room is stranded; it does NOT unstrand it. If a queen crashes between `confirm-merge` approval and `gh pr merge --squash`, or if `gh pr merge --squash` fails for any reason, the room remains `closed + decision_outcome: merge_approved + github_merge_status: pending` until a human intervenes. PR 3 ships a **server-side reconciler** that mirrors G30's trigger logic and *acts*: every N seconds, scan rooms with that signature older than 2 min, read PR state via the App-installation token (`gh pr view --json state,mergeCommit.oid` server-side, or GraphQL equivalent), and write `github_merge_status: succeeded | failed`. Bounded by G4's 15-min TTL on `decided_pending_action` and an additional 15-min reconcile window on the post-`closed` stranded state. **Hard PR 3 review gate**: if PR 3 ships detection-only (no reconciler) and `queen_mode=local` ever ships to production, the v1 design must revert to post-merge close + symmetric `seal-merge` callback. Don't let the dashboard alone be the answer.
