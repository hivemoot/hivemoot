...
> PR workflows are disabled by default. Add a `pr:` section under `governance:` to opt in.
...
| `governance.pr.automerge.allowedPaths` | `string[]` | `["**/*.md", "**/*.txt", "docs/**"]` | Allowlist of file globs that every changed path must match for automerge eligibility. |
| `governance.pr.automerge.denyPaths` | `string[]` | built-in denylist | Denylist of file globs that immediately disqualify a PR from automerge. Legacy alias `excludedPaths` is also accepted for backward compatibility. |
