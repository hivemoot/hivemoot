...
class LegacySettingsShimTests(unittest.TestCase):
    """Unit tests for engine._build_legacy_settings.

    The shim bridges raw YAML values into the settings dict for
    plugins that haven't moved to ``config.typed`` yet.  Tests pin
    the conversion rules and the YAML-wins precedence so future
    changes are explicit.
    """

    def _shim(self, raw):
        from hivemoot_agent.engine import _build_legacy_settings
        return _build_legacy_settings(raw)

    def test_bool_becomes_one_or_zero(self):
        s = self._shim({"watch_mentions": True, "watch_review_requests": False})
        self.assertEqual(s["watch_mentions"], "1")
        self.assertEqual(s["WATCH_MENTIONS"], "1")
        self.assertEqual(s["watch_review_requests"], "0")
        self.assertEqual(s["WATCH_REVIEW_REQUESTS"], "0")

    def test_int_and_float_are_stringified(self):
        s = self._shim({"clone_depth": 50, "ratio": 1.5})
        self.assertEqual(s["clone_depth"], "50")
        self.assertEqual(s["CLONE_DEPTH"], "50")
        self.assertEqual(s["ratio"], "1.5")

    def test_list_of_scalars_is_comma_joined(self):
        s = self._shim({"repos": ["acme/api", "acme/web"]})
        self.assertEqual(s["repos"], "acme/api,acme/web")
        self.assertEqual(s["REPOS"], "acme/api,acme/web")

    def test_dict_values_are_skipped(self):
        s = self._shim({"nested": {"inner": "val"}, "simple": "kept"})
        self.assertNotIn("nested", s)
        self.assertNotIn("NESTED", s)
        self.assertEqual(s["simple"], "kept")

    def test_list_of_dicts_is_skipped(self):
        s = self._shim({"schedules": [{"name": "a"}, {"name": "b"}]})
        self.assertNotIn("schedules", s)
        self.assertNotIn("SCHEDULES", s)

    def test_yaml_wins_over_env_for_both_cases(self):
        os.environ["HMT_SHIM_KEY"] = "env-value"
        os.environ["HMT_SHIM_KEY".lower()] = "env-lower"  # unlikely but covered
        try:
            s = self._shim({"HMT_SHIM_KEY": "yaml-value"})
        finally:
            del os.environ["HMT_SHIM_KEY"]
            os.environ.pop("HMT_SHIM_KEY".lower(), None)
        # YAML wins for the exact key:
        self.assertEqual(s["HMT_SHIM_KEY"], "yaml-value")

    def test_yaml_wins_over_env_for_uppercase_alias(self):
        os.environ["GITHUB_REPOS"] = "env-repo"
        try:
            s = self._shim({"repos": ["yaml/one", "yaml/two"]})
        finally:
            del os.environ["GITHUB_REPOS"]
        self.assertEqual(s["repos"], "yaml/one,yaml/two")
        # The uppercase alias comes from the YAML key ``repos`` (→ ``REPOS``),
        # not from a matching env var — so GITHUB_REPOS stays as the env
        # value because YAML didn't write that specific alias.  But if YAML
        # provides a ``github_repos`` key, it overwrites GITHUB_REPOS:
        os.environ["GITHUB_REPOS"] = "env-again"
        try:
            s = self._shim({"github_repos": "yaml/override"})
        finally:
            del os.environ["GITHUB_REPOS"]
        self.assertEqual(s["GITHUB_REPOS"], "yaml/override")
